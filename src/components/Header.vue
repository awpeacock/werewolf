...
const updateState = (path: string) => {
	...
	header.value = isHome.value ? styles.header.home : styles.header.default;
	image.value = isHome.value ? styles.image.home : styles.image.default;
};
...
<template>
	<header class="flex flex-col w-full text-center my-4">
		<Head>
			<Title>{{
		...
		<div class="flex" :class="header">
			<h1 v-if="isHome" class="text-7xl text-yellow-200 font-oswald">{{ $t('werewolf') }}</h1>
